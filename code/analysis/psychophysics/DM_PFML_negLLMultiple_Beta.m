...
if (min(params(:,4)) < lapseLimits(1)) || (max(params(:,4)) > lapseLimits(2)) || (eta >= 1) || (eta < 0)
    negLL = Inf;    
else
    ...
    for c = 1:nConditions
        ...
        if betaBool && (eta^2 > 10^-9) 
            ...
            LLcond(c) = nansum(gammaln(NumPos(c,:)+alpha)+gammaln((OutOfNum(c,:)-NumPos(c,:))+beta)-gammaln(OutOfNum(c,:)+eta_prime)+gammaln(eta_prime)-gammaln(alpha)-gammaln(beta));
        else
            %binomial model
