function [out,M] = run_pcm_fMRI_pre_winning_pmf(varargin)
...
if ~ismember('fitRes',p.UsingDefaults)
    ...
    % Psychometric funtion NO SHIFT
    Ac = featPmfPre';          % Pretest
    theta0     = ones(size(Ac,3),1);
    Gc_pmf_noshift = getGmatrix(Ac,theta0,nConds,'center',centerG);
    
    % Model 2: Hemifield_noshift+Decisional_noshift
    M{2}.type       = 'component';
    M{2}.numGparams = 2;
    M{2}.Gc(:,:,1)  = Gc_hemi_noshift;
    M{2}.Gc(:,:,2)  = Gc_dec_noshift;
    M{2}.name       = 'HemiNoSh+DecNoSh';
    
    % Model 3: Hemifield_noshift+Pmf_noshift
    M{3}.type       = 'component';
    M{3}.numGparams = 3;
    M{3}.Gc(:,:,1)  = Gc_hemi_noshift;
    M{3}.Gc(:,:,2)  = Gc_dec_noshift;
    M{3}.Gc(:,:,3)  = Gc_pmf_noshift;
    M{3}.name       = 'HemiNoSh+DecNoSh+PmfNoSh';
    
    % Model 4: Free model as Noise ceiling
    if strcmp(freeType,'freechol')
        M{4}.type      = 'freechol';
        M{4}.numCond   = nConds;
        M{4}.name      = 'noiseceiling';
        M{4}           = pcm_prepFreeModel(M{4});
    else
        M{4}.type       = 'freedirect';
        M{4}.numGparams = 0;
        M{4}.name       = 'noiseceiling';
