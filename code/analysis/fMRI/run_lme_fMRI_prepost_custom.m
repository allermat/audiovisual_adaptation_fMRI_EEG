function varargout = run_lme_fMRI_prepost_custom(modelNames,varargin)
...
addParameter(p,'modelColors',{},@(x) validateattributes(x,{'cell'},{'ncols',5}));
...
for iHemi = 1:numel(hemiToLoad)
    ...
    x_Dec_noshift = repmat([x_Dec_pre; x_Dec_pre; x_Dec_pre], nsubjects*nROIs, 1);
    x_Sp_recal = repmat([x_Sp_pre; x_Sp_postAV; x_Sp_postVA], nsubjects*nROIs, 1);
    x_Sp_noshift = repmat([x_Sp_pre; x_Sp_pre; x_Sp_pre], nsubjects*nROIs, 1);
    x_Pmf_recal = repmat([x_Pmf_pre; x_Pmf_postAV; x_Pmf_postVA], nsubjects*nROIs, 1);
    x_Pmf_noshift = repmat([x_Pmf_pre; x_Pmf_pre; x_Pmf_pre], nsubjects*nROIs, 1);
    
    % Generating lookup table for predictiors
    name = {'Const';'SpRecal';'Sp';'DecRecal';'Dec';'PmfRecal';'Pmf'};
    X = {ones(nsubjects*nlocs*nconds*nROIs,1);x_Sp_recal;x_Sp_noshift;...
        x_Dec_recal;x_Dec_noshift;x_Pmf_recal;x_Pmf_noshift};
...
if plotFigure
    ...
    for iFig = 1:nFigs
        figure();
        set(gcf, 'Position', [0 0 1000 250], 'PaperPositionMode', 'auto');
        for iROI = 1:nROIs
