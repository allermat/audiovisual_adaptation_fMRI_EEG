function [out,M] = run_pcm_fMRI_prepost_winning_pmf(varargin)
...
if ~ismember('fitRes',p.UsingDefaults)
    ...
    % Hemifield with two populations
    % Right hemisphere
    ...
    % Hemifield NO SHIFT with two populations
    [Ac,Ac_pre,Ac_postVA,Ac_postAV] = deal([]);
    for i = 1:size(featHemiPre,1)
        % Predicted response for each location for right hemifield neurons
        Ac_pre(:,i,i)     = [featHemiPre(i,:),zeros(1,14)]';          % Pretest
    ...
    Ac = cat(3,Ac_pre,Ac_postVA,Ac_postAV);
    theta0 = ones(size(Ac,3),1);
    Gc_hemi_noshift = getGmatrix(Ac,theta0,nConds,'center',centerG);
    ...
    Ac = cat(3,Ac_pre,Ac_postVA,Ac_postAV);
    theta0       = ones(size(Ac,3),1);
    Gc_dec_noshift      = getGmatrix(Ac,theta0,nConds,'center',centerG);
    
    % Psychometric funtion model
    [Ac,Ac_pre,Ac_postVA,Ac_postAV] = deal([]);
    ...
    % Psychometric funtion NO SHIFT model
    [Ac,Ac_pre,Ac_postVA,Ac_postAV] = deal([]);
    Ac_pre     = [featPmfPre,zeros(1,14)]';          % Pretest
    ...
    Ac = cat(3,Ac_pre,Ac_postVA,Ac_postAV);
    theta0     = ones(size(Ac,3),1);
    Gc_pmf_noshift = getGmatrix(Ac,theta0,nConds,'center',centerG);

    % Model 2: Hemifield+Decisional
    M{2}.type       = 'component';
    M{2}.numGparams = 2;
    M{2}.Gc(:,:,1)  = Gc_hemi;
    M{2}.Gc(:,:,2)  = Gc_dec;
    ...
    % Model 3: Hemifield+Decisional+Pmf
    M{3}.type       = 'component';
    M{3}.numGparams = 3;
    M{3}.Gc(:,:,1)  = Gc_hemi;
    M{3}.Gc(:,:,2)  = Gc_dec;
    M{3}.Gc(:,:,3)  = Gc_pmf;
    M{3}.name       = 'Hemi+Dec+Pmf';
    
    % Model 4: Hemifield+Decisional+PmfNoShift
    M{4}.type       = 'component';
    M{4}.numGparams = 3;
    M{4}.Gc(:,:,1)  = Gc_hemi;
    M{4}.Gc(:,:,2)  = Gc_dec;
    M{4}.Gc(:,:,3)  = Gc_pmf_noshift;
    M{4}.name       = 'Hemi+Dec+PmfNoSh';
    
    % Model 5: Hemifield_noshift+Decisional_noshift
    M{5}.type       = 'component';
    M{5}.numGparams = 2;
    M{5}.Gc(:,:,1)  = Gc_hemi_noshift;
    M{5}.Gc(:,:,2)  = Gc_dec_noshift;
    M{5}.name       = 'HemiNoSh+DecNoSh';
    
    % Model 6: Hemifield_noshift+Decisional_noshift+PmfNoShift
    M{6}.type       = 'component';
    M{6}.numGparams = 3;
    M{6}.Gc(:,:,1)  = Gc_hemi_noshift;
    M{6}.Gc(:,:,2)  = Gc_dec_noshift;
    M{6}.Gc(:,:,3)  = Gc_pmf_noshift;
    M{6}.name       = 'HemiNoSh+DecNoSh+PmfNoSh';
    
    % Model 7: Hemifield+Decisional_noshift
    M{7}.type       = 'component';
    M{7}.numGparams = 2;
    M{7}.Gc(:,:,1)  = Gc_hemi;
    M{7}.Gc(:,:,2)  = Gc_dec_noshift;
    M{7}.name       = 'Hemi+DecNoSh';
    
    % Model 8: Hemifield+Decisional_noshift+Pmf
    M{8}.type       = 'component';
    M{8}.numGparams = 3;
    M{8}.Gc(:,:,1)  = Gc_hemi;
    M{8}.Gc(:,:,2)  = Gc_dec_noshift;
    M{8}.Gc(:,:,3)  = Gc_pmf;
    M{8}.name       = 'Hemi+DecNoSh+Pmf';
    
    % Model 9: Hemifield+Decisional_noshift+Pmf_noshift
    M{9}.type       = 'component';
    M{9}.numGparams = 3;
    M{9}.Gc(:,:,1)  = Gc_hemi;
    M{9}.Gc(:,:,2)  = Gc_dec_noshift;
    M{9}.Gc(:,:,3)  = Gc_pmf_noshift;
    M{9}.name       = 'Hemi+DecNoSh+PmfNoSh';
    
    % Model 10: Free model as Noise ceiling
    if strcmp(freeType,'freechol')
        M{10}.type      = 'freechol';
        M{10}.numCond   = nConds;
        M{10}.name      = 'noiseceiling';
        M{10}           = pcm_prepFreeModel(M{10});
    else
        M{10}.type       = 'freedirect';
        M{10}.numGparams = 0;
        M{10}.name       = 'noiseceiling';
        M{10}.theta0     = [];
    end
