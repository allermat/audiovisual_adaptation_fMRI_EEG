function [out,M] = run_pcm_fMRI_pre(varargin)
...
if ~ismember('fitRes',p.UsingDefaults)
    ...
    % Model 3: Decisional no recalibration
    Ac = [];
    ...
    if strcmp(thirdModel,'Pmf')
        
        % Model 5: Psychometric funtion no recalibration
        ...
        % Model 7: Decisional+Pmf
        M{iModel}.type       = 'component';
        M{iModel}.numGparams = 2;
        M{iModel}.Gc(:,:,1)  = Gc_dec;
        M{iModel}.Gc(:,:,2)  = Gc_pmf;
        M{iModel}.name       = 'Dec+Pmf';
        iModel = iModel+1;
        
        % Model 8: Hemifield+Decisional+Pmf
        M{iModel}.type       = 'component';
