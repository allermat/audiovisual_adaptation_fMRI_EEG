...
if ismember(trMethod,{'sample-wise','sample-wise-avg','sample-wise-beta',...
        ...
        'sample-wise-sm-corr1','sample-wise-sm-corr2','sample-wise-rl','sample-wise-sc',...
        'sample-wise-mult','sample-wise-sm','sample-wise-sm-pseudoav'})
    
    if ismember(trMethod,{'sample-wise-beta','sample-wise-beta-rl','sample-wise-sm-beta'})
        if ismember(trLabel,{'aloc','vloc'})
            featStr = 'feat_stim';
...
elseif ismember(trMethod,{'sample-wise-tf','sample-wise-tf-avg'})
    ...
    if ismember(tfType,{'p','ph'})
        ...
        if ~ismember(featStr,fields)
            error('mvpa:extractfeatures:missingFeatureType',...
    ...
    elseif strcmp(tfType,'pph')
        
        featStr = {'feat_p','feat_ph'};
        fields = whos(dataM);
        fields = {fields.name}';
        if any(~ismember(featStr,fields))
            error('mvpa:extractfeatures:missingFeatureType',...
                  'The specified feature is missing from the dataset.');
        end
