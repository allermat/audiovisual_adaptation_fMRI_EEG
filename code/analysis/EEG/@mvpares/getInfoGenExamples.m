...
if isfield(obj.info,'tr_nRepsAvg')
    genExamples = repmat(genExamples,obj.info.tr_nRepsAvg,1);
end
    ...
    switch cvScheme
        case 'kf'
            ...
            for i = 1:numel(cvFold)
                ...
                actGrouping = actGrouping(:);
                actFoldExamples = ismember(actGrouping,[iKFfold,commonGroupLabel]);
        ...
        case {'loso','loro'}
            for i = 1:numel(cvFold)
                actFoldExamples = ismember(groupings(:),[cvFold(i),commonGroupLabel]);
                temp = genExamples(actFoldExamples,:);
                temp.fold = ones(size(temp,1),1)*cvFold(i);
                genExamplesInfo = cat(1,genExamplesInfo,temp);
            end
