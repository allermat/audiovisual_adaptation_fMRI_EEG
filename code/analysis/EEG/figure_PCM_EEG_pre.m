function logBF_table = figure_PCM_EEG_pre(dataPath,varargin)
...
if isempty(thirdModel)
    ...
    M = dataRand.M;
    nTimeWin = size(out,1);
    plotOrder = repmat({2:4},1,nTimeWin);
    modelNames = {{'Sp','Dec','Sp+Dec'}};
    if pltIndiv
        yBreaks = {[],[],[],[]};
        yLims = {[0,350],[-150,450],[-110,325],[-25,250]};
    else
        yBreaks = {[],[],[],[]};
        yLims = {[],[],[],[]};
    end
elseif strcmp(thirdModel,'Pmf')
    ...
    M = dataRand.M;
    nTimeWin = size(out,1);
    plotOrder = repmat({2:8},1,nTimeWin);
