function logBF_table = figure_PCM_EEG_prepost_winning_pmf(dataPath,varargin)
...
M = dataRand.M;
nTimeWin = size(out,1);
plotOrder = {[2,4],[5,7],[8,10],[8,10]};
modelNames = {{'Sp+Dec','Sp+Dec+PmfRecal'},...
              {'SpRecal+Dec','SpRecal+Dec+PmfRecal'},...
...
for iPlot = 1:nPlot
    ...
    Tgroup = out{iPlot,1};
    Tcross = out{iPlot,2};
    nullModel = 2; % Corresponing to Sp+Dec
