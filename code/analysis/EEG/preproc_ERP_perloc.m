function preproc_ERP_perloc(subID)
...
fprintf('\n\nAveraging tials...\n');
parfor_progress(size(conditions,1));

for iCond = 1:size(conditions,1)
    
    actCond = conditions(iCond,:);
    ...
    if ismember(actCond.blocktype,'pre-test')
        cfg.trials = cond == actCond.condition & badTrials == 0 & ...
            ~catchTrials;
    else
        cfg.trials = cond == actCond.condition & badTrials == 0 & ...
            ismember(adaptdir,actCond.adaptdir) & ~catchTrials;
    end
    ...
    locStr = num2str(round(actCond.aloc));
